// BodyApp.tsx
export default function BodyApp({data,setData,forms,setForms}) {
    ...
    const handleSubmit = (e) => {
        ...
        const dataBaru = {
            id: new Date().toISOString() , // atau pakai Date.now(),
    ...
    const handleFinish = (datas) => {

        const dataUpdate = data.map((item) => {
            return datas.id === item.id ? {...item, isKelar: !item.isKelar} : item
        });
        setData(dataUpdate)
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {data.map((datas) => (
                        <div className="bg-white border border-slate-300 rounded-lg p-5 shadow-sm" key={datas.id}>
                            <h4 className={`text-lg font-semibold  ${datas.isKelar ? "text-slate-800 line-through": '' }  `}>{datas.aktivitas}</h4>
                            <p className="text-slate-600 mb-4">{datas.deskripsi}</p>
                            <div className="flex justify-between">
                                <button className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded"
                                onClick={() => handleFinish(datas)}
                                >
                                    Selesai
                                </button>
